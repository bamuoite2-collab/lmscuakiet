...
-- Creates tables and functions for managing seasonal events, themed quests, and special challenges
...
create table if not exists event_quests (
  ...
  title text not null,
  description text,
  quest_type text, -- 'complete_lessons', 'maintain_streak', 'achieve_score', etc.
...
create policy "Only admins can manage events" on seasonal_events for all using (
  exists (select 1 from profiles where id = auth.uid() and role = 'admin')
);

-- event_quests: public read
...
create policy "Only admins can manage event quests" on event_quests for all using (
  exists (select 1 from profiles where id = auth.uid() and role = 'admin')
);
...
begin
  ...
  if not found then
    insert into user_event_progress (user_id, event_id, event_quest_id, target_value, current_progress)
    values (p_user_id, v_quest.event_id, p_quest_id, v_quest.target_value, v_quest.target_value)
...
-- Update event progress (called when user completes relevant action)
...
-- Sample Tết Event
insert into seasonal_events (name, description, event_type, start_date, end_date, is_active, theme_config, bonus_xp_multiplier, icon)
values (
  'Tết Nguyên Đán 2025',
  ...
  false, -- Will be activated when date comes
  '{
    "primaryColor": "#d4af37",
  ...
  1.5,
  '🧧'
) returning id as tet_event_id \gset

-- Tết Quests
insert into event_quests (event_id, title, description, quest_type, target_value, xp_reward, difficulty, icon, order_index)
values
  (:'tet_event_id', 'Chăm Chỉ Đầu Năm', 'Hoàn thành 10 bài học trong sự kiện Tết', 'complete_lessons', 10, 200, 'medium', '📚', 1),
  (:'tet_event_id', 'Streak Đầu Xuân', 'Giữ streak liên tục 7 ngày', 'maintain_streak', 7, 300, 'hard', '🔥', 2),
  (:'tet_event_id', 'Thủ Khoa Năm Mới', 'Đạt điểm 90%+ trong 5 bài quiz', 'high_score', 5, 250, 'hard', '🌟', 3),
  (:'tet_event_id', 'Tích Cực Học Tập', 'Học ít nhất 2 giờ trong tuần', 'study_time', 120, 150, 'easy', '⏰', 4);

-- Event Achievements
insert into event_achievements (event_id, code, title, description, icon, xp_reward, criteria)
values
  (:'tet_event_id', 'tet_2025_complete_all', '🧧 Lì Xì May Mắn', 'Hoàn thành tất cả nhiệm vụ Tết', '🧧', 500, '{"type": "complete_all_quests"}'::jsonb),
  (:'tet_event_id', 'tet_2025_top10', '🏮 Đèn Lồng Vàng', 'Lọt top 10 bảng xếp hạng Tết', '🏮', 1000, '{"type": "leaderboard_rank", "max_rank": 10}'::jsonb);

-- Grant permissions
grant usage on schema public to authenticated;
grant all on seasonal_events to authenticated;
grant all on event_quests to authenticated;
grant all on user_event_progress to authenticated;
grant all on event_achievements to authenticated;
grant all on user_event_achievements to authenticated;
